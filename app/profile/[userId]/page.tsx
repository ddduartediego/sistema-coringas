...
import CobrancasIntegrante from '../components/CobrancasIntegrante'; // Adjusted import path
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (!targetUserId) {
          console.error('User ID não encontrado nos parâmetros da rota.');
          setMessage({ type: 'error', text: 'Perfil não encontrado.' });
          setLoading(false);
          return;
        }
        
        // Buscar perfil do targetUserId
        ...
        if (profileError) {
          console.error(`Erro ao buscar perfil para ${targetUserId}:`, profileError.message);
          setMessage({ type: 'error', text: `Erro ao carregar o perfil: ${profileError.message}` });
          setProfileData(null);
          setLoading(false);
          return;
        }
        ...
        if (!profile) {
          console.error(`Perfil não encontrado para ${targetUserId}. A query retornou nulo.`);
          setMessage({ type: 'error', text: 'Perfil não encontrado.' });
          setProfileData(null);
          setLoading(false);
          return;
        ...
        // Atualizar userData com os dados do perfil visualizado
        // userData é usado para o cabeçalho da página (nome, email, avatar)
        setUserData({
            id: profile.user_id, // ID do perfil visualizado
    ...
    fetchData();
  }, [supabase, setValue, params?.userId, setFocus]);
  ...
  const onSubmit = async (data: ProfileFormData) => {
    try {
      ...
      if (!profileData?.id) return; // Usa o ID do perfil carregado (profileData.id)
      ...
      const { error } = await supabase
        ...
        .eq('id', profileData.id); // Condição de update usa profileData.id
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          >
            <div className="p-6">
              <CobrancasIntegrante 
                supabase={supabase} 
                userId={profileData?.user_id || params?.userId}
              />
            </div>
          </motion.div>
        </div>
